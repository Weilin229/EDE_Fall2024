...
EPAair <- read.csv(here("~/EDE_Fall2024/Data/Processed_KEY/NEON_NIWO_Litter_mass_trap_Processed.csv"), stringsAsFactors = TRUE)
# Set date to date format
EPAair$collectDate <- as.Date(EPAair$collectDate, format = "%Y-%m-%d")

Litter <- read.csv(here("~/EDE_Fall2024/Data/Processed_KEY/NEON_NIWO_Litter_mass_trap_Processed.csv"), stringsAsFactors = TRUE)
# Set date to date format
...
```{r}
# Density Plot
ggplot(EPAair, aes(x = Ozone, fill = year)) +
  geom_density(alpha = 0.5) +  # Add transparency
  scale_fill_brewer(palette = "Set2") +  # Use a non-default color palette
  labs(
    title = "Density of Ozone Levels by Year",
    x = "Ozone (ppb)",
    y = "Density",
    fill = "Year"
  ) +
